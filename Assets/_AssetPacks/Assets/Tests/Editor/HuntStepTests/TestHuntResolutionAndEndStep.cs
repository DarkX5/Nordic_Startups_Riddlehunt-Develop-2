...
[TestFixture]
public class TestHuntResolutionAndEndStep
{
    ...
    public void SetupTextGetterMocks(HuntAsset answerAsset, string output)
    {
        ...
        _endAsset = new HuntAsset() {Type = AssetType.EndText, Url = _endUrl};
        _videoAsset = new HuntAsset() { Type = AssetType.VideoToPlay, Url = _videoUrl };
        _step = new HuntStep() { Type = StepType.HuntResolutionAndEnd, Assets = new List<HuntAsset>() { _storyAsset, _riddleAsset, _answerAsset, _endAsset, _videoAsset } };
        ...
        _textGetterMock
            .Setup(x => x.GetText(_endUrl, false, It.IsAny<Action<string>>()))
            .Callback<string, bool, Action<string>>((myEnd, myCache, myAction) =>
            {
                myAction.Invoke(_endText);
            }).Verifiable();
    ...
    public void Init()
    {
        ...
        _endUrl = "http://www.endUrl.com";
        _videoUrl ="http://www.videoUrl.com/mp4";
    ...
    public void TestNewInstance_wrong_type_throws()
    {
        ...
        //Arrange
        _step = new HuntStep() { Type = StepType.DisplayRiddleAndSubmitAnswer, Assets = new List<HuntAsset>() { _storyAsset, _riddleAsset } };

        //Act and Assert
        Assert.Throws<ArgumentException>(() => new HuntResolutionAndEndStep(_step, _textGetterMock.Object, _imageGetterMock.Object, readyAction));
    }

    [Test]
    public void TestGetStepType()
    {
        //Given a new HuntResolutionAndEndStep with corresponding type
        //When GetStepType is called
        //returns correctStepType
        
        //Arrange
        var sut = new HuntResolutionAndEndStep(_step, _textGetterMock.Object, _imageGetterMock.Object, readyAction);
        //Act
        var stepType = sut.GetStepType();
        //Assert
        Assert.AreEqual(StepType.HuntResolutionAndEnd, stepType);
    }

    [Test]
    public void TestGetStoryText()
    {
        //Given a new HuntResolutionAndEndStep
    ...
    public void TestGetEndText()
    {
        ...
        //Then the EndText element is returned.
        //Arrange
        var sut = new HuntResolutionAndEndStep(_step, _textGetterMock.Object, _imageGetterMock.Object, readyAction);
        //Act
        var endText = sut.GetEndText();
        //Assert
        Assert.AreEqual(_endText, endText);
    }
    [Test]
    public void TestGetVideoUrl()
    {
        //Given a new HuntResolutionAndEndStep
        //When GetVideoUrl is called
        //Then the videoUrl element is returned.
        //Arrange
        var sut = new HuntResolutionAndEndStep(_step, _textGetterMock.Object, _imageGetterMock.Object, readyAction);
        //Act
        var videoUrl = sut.GetVideoUrl();
        //Assert
        Assert.AreEqual(_videoUrl, videoUrl);
    }
    [Test]
    ...
    {
        //Given a new HuntResolutionAndEndStep and a huntstep containing an imagelist huntAsset with 1 imagelink
